...
tape( 'the function has an arity of 1', function test( t ) {
	t.strictEqual( dtrsv.length, 1, 'has expected arity' );
	t.end();
});

tape( 'the function solves the system of equations `A*x = b` (upper, no-transpose, non-unit)', function test( t ) {
	var expected;
	var trans;
	var diag;
	var uplo;
	...
	Abuf = new Float64Array( [ 1.0, 2.0, 3.0, 0.0, 4.0, 5.0, 0.0, 0.0, 6.0 ] );
	x = vector( xbuf, 3, 1, 0 );
	A = matrix( Abuf, 3, 3, 1, 0 );

	uplo = scalar2ndarray( resolveTriangle( 'upper' ), {
		'dtype': 'int32'
	});
	trans = scalar2ndarray( resolveTranspose( 'no-transpose' ), {
		'dtype': 'int32'
	});
	diag = scalar2ndarray( resolveDiagonal( 'non-unit' ), {
		'dtype': 'int32'
	});

	v = dtrsv( [ A, x, uplo, trans, diag ] );

	expected = new Float64Array( [ -0.25, -0.125, 0.5 ] );
...
tape( 'the function solves the system of equations `A*x = b` (lower, no-transpose, non-unit)', function test( t ) {
	var expected;
	var trans;
	var diag;
	var uplo;
	...
	xbuf = new Float64Array( [ 1.0, 3.0, 5.0 ] );
	Abuf = new Float64Array( [ 1.0, 0.0, 0.0, 2.0, 4.0, 0.0, 3.0, 5.0, 6.0 ] );
	x = vector( xbuf, 3, 1, 0 );
	A = matrix( Abuf, 3, 3, 1, 0 );

	uplo = scalar2ndarray( resolveTriangle( 'lower' ), {
		'dtype': 'int32'
	});
	trans = scalar2ndarray( resolveTranspose( 'no-transpose' ), {
		'dtype': 'int32'
	...
	expected = new Float64Array( [ 1.0, 0.25, 0.125 ] );
	t.strictEqual( v, x, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
...
tape( 'the function solves the system of equations `A^T*x = b` (upper, transpose, non-unit)', function test( t ) {
	var expected;
	var trans;
	var diag;
	var uplo;
	...
	trans = scalar2ndarray( resolveTranspose( 'transpose' ), {
		'dtype': 'int32'
	});
	diag = scalar2ndarray( resolveDiagonal( 'non-unit' ), {
		'dtype': 'int32'
...
tape( 'the function supports a unit diagonal (upper, no-transpose, unit)', function test( t ) {
	var expected;
	var trans;
	var diag;
	var uplo;
	...
	xbuf = new Float64Array( [ 1.0, 2.0, 3.0 ] );
	Abuf = new Float64Array( [ 1.0, 2.0, 3.0, 0.0, 1.0, 5.0, 0.0, 0.0, 1.0 ] );
	x = vector( xbuf, 3, 1, 0 );
	A = matrix( Abuf, 3, 3, 1, 0 );
	...
	diag = scalar2ndarray( resolveDiagonal( 'unit' ), {
		'dtype': 'int32'
	});

	v = dtrsv( [ A, x, uplo, trans, diag ] );

	expected = new Float64Array( [ 18.0, -13.0, 3.0 ] );
	t.strictEqual( v, x, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
...
tape( 'the function supports ndarrays having non-unit strides', function test( t ) {
	...
	expected = new Float64Array( [ -0.25, 0.0, -0.125, 0.0, 0.5 ] );
	t.strictEqual( v, x, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
	t.end();
});
...
tape( 'the function supports ndarrays having negative strides', function test( t ) {
	...
	expected = new Float64Array( [ 0.5, -0.125, -0.25 ] );
	t.strictEqual( v, x, 'returns expected value' );
	t.strictEqual( isSameFloat64Array( getData( v ), expected ), true, 'returns expected value' );
	t.end();
});
